"""
Modèles Pydantic pour le connecteur Mistral AI
"""
...
class ChatMessage(BaseModel):
    ...
    images: Optional[List[str]] = Field(None, description="Liste d'images en base64 ou URLs (pour les modèles vision comme Pixtral)")
...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(None, description="Modèle Mistral à utiliser (par défaut: mistral-small-latest)")
    ...
    safe_prompt: Optional[bool] = Field(False, description="Activer le mode safe prompt")
...
class EmbeddingRequest(BaseModel):
    ...
    model: Optional[str] = Field("mistral-embed", description="Modèle d'embedding à utiliser")
...
class HealthResponse(BaseModel):
    ...
    mistral_configured: bool = Field(..., description="Indique si la clé API Mistral est configurée")
