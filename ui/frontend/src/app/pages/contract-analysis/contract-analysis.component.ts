...
/**
 * Page principale de l'agent d'analyse de contrats
 */
@Component({
...
export class ContractAnalysisComponent implements OnInit {
  ...
  loadConfig(): void {
    const savedConfig = localStorage.getItem('contract-analyzer-config');
    if (savedConfig) {
      ...
      this.openaiModel = config.openaiModel || 'gpt-4-turbo-preview';
  ...
  async analyzeContract(): Promise<void> {
    ...
    if (this.provider === 'mistral' && !this.mistralApiKey) {
      this.errorMessage = 'Veuillez saisir votre clé API Mistral';
      return;
    }

    if (this.provider === 'openai' && !this.openaiApiKey) {
      this.errorMessage = 'Veuillez saisir votre clé API OpenAI';
      return;
    }
    ...
    try {
      ...
      if (this.provider === 'mistral') {
        ...
        formData.append('openai_api_key', this.openaiApiKey);
        formData.append('openai_model', this.openaiModel);
      }
      ...
      const response = await firstValueFrom(
        this.http.post<ContractAnalysisResult>(
          `${environment.api.contractAnalysis}/api/v1/analyze/contract`,
          formData
        )
      );
      ...
      if (!response.success) {
        this.errorMessage = response.message || 'Une erreur est survenue lors de l\'analyse';
      }

    } catch (error: any) {
      this.errorMessage = error.error?.detail || error.message || 'Erreur de connexion au serveur';
  ...
  async downloadWordSynthesis(): Promise<void> {
    ...
    try {
      ...
      const downloadUrl = `${environment.api.contractAnalysis}/api/v1/analyze/files/${fileId}`;
      ...
      link.href = url;
      link.download = `analyse_contrat_${this.analysisResult.analysis_id}.docx`;
      ...
      this.errorMessage = 'Erreur lors du téléchargement du fichier Word';
    }
  }
  ...
  get canAnalyze(): boolean {
    ...
    return this.uploadedFiles.length > 0 && hasApiKey && !this.isAnalyzing;
  }
}
