"""
Modèles Pydantic pour le connecteur OpenAI
"""
...
class ChatMessage(BaseModel):
    ...
    images: Optional[List[str]] = Field(None, description="Liste d'images en base64 ou URLs (pour les modèles vision comme GPT-4o)")
...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(None, description="Modèle OpenAI à utiliser (par défaut: gpt-3.5-turbo)")
...
class EmbeddingRequest(BaseModel):
    ...
    model: Optional[str] = Field("text-embedding-ada-002", description="Modèle d'embedding à utiliser")
...
class ModelInfo(BaseModel):
    """Informations sur un modèle OpenAI"""
...
class HealthResponse(BaseModel):
    ...
    openai_configured: bool = Field(..., description="Indique si la clé API OpenAI est configurée")
