"""
Modèles Pydantic pour le connecteur Perplexity
"""
...
class ChatMessage(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "role": "user",
                "content": "Quelles sont les dernières nouvelles sur l'IA ?"
            }
        }
...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(None, description="Modèle Perplexity à utiliser")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "messages": [
                    {"role": "user", "content": "Quelles sont les dernières avancées en IA ?"}
                ],
                "model": "sonar",
                "temperature": 0.7,
                "max_tokens": 500
...
class ModelInfo(BaseModel):
    """Informations sur un modèle Perplexity"""
...
class HealthResponse(BaseModel):
    ...
    perplexity_configured: bool = Field(..., description="Indique si la clé API Perplexity est configurée")
